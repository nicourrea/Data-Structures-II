...
{
   TListIterator<T> itr = t2.GetIterator(); // create iterator for t2

   TList<T> result = t1; // copy t1 to result

   if (t2.GetSize() == 0) return result; // if t2 is empty, return t1

   for (int i = 0; i < t2.GetSize(); i++) // iterate through t2
   {
      result.InsertBack(itr.GetData()); // insert data from t2
      itr.Next(); // increment iterator
   }
